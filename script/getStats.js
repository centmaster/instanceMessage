        ...
        (function getPrivateStats() {
		    ...
		    vconnection: {},
                ...
                for (var i = 0; i < results.length; ++i) {
                    ...
                    if(res.googActiveConnection == 'true' && res.googChannelId == "Channel-audio-2") {
                        result.aconnection = merge(result.aconnection, {
                            channelId: res.googChannelId,
                            read: res.googReadable,
                            write: res.googWritable,
                            bytesReceived: res.bytesReceived
                        });
			
                    }

                    if(res.googActiveConnection == 'true' && res.googChannelId == "Channel-video-2") {
                        result.vconnection = merge(result.vconnection, {
                            channelId: res.googChannelId,
                            read: res.googReadable,
                            write: res.googWritable,
                            bytesReceived: res.bytesReceived
